...
/**
 * `--first-run` emits the same organisation before its first measurement: the register of
 * people, the groups and the duties, a wheel that is described but not switched on, and
 * nothing measured — no measurement, round, measure, comment or risk assessment, and no
 * register fetch. It is the state a daglig leder meets the Veiviser in (P7, D-76), and the
 * only honest way to put the fixture there: the same file, the same ids, and a plain rerun
 * without the flag brings the design's scenario back.
 *
 *   node scripts/seed/design-fixture.mjs --first-run > /tmp/first-run.sql
 */
...
delete from app.setup_progress  where org_id = '${ORG}';
...
${FIRST_RUN ? '  null, null, null, null, null, null, null, null, null, null, null)' : `  '${REGISTRY.fetched}', ${q(REGISTRY.form[0])}, ${q(REGISTRY.form[1])},
...
values ('${ORG}', ${!FIRST_RUN}, '${WHEEL.cadence}', ${WHEEL.baselineMonth}, ${WHEEL.leadDays},
...
${FIRST_RUN ? '-- first run: a new organisation has no notice ladder until the Veiviser writes one' : `insert into app.wheel_notifications (wheel_id, audience, lead_days, sort_order)
...
${FIRST_RUN ? `select count(*) as employees from app.employees where org_id = '${ORG}';` : ''}`)

if (!FIRST_RUN) console.log(`insert into app.measurements (id, org_id, kind, year, label) values
