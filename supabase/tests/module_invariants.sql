...
--   * a round's modules: another organisation cannot read them; fixed once open (9, 10)
...
--
-- It needs the module seeded (npm run -s modules:seed | psql), as CI does after migrations.
...
begin
  select m.id, m.content_hash into v_mod, v_hash from app.question_modules m where m.key = 'bygg-og-anlegg' and m.version = '1.0.0';
  if v_mod is null then
    raise exception 'module_invariants: bygg-og-anlegg@1.0.0 is not seeded (npm run -s modules:seed | psql)';
  end if;
  ...
  begin
    ...
    update app.question_modules set status = 'draft', published_at = null, retired_at = null where id = v_mod and status = 'draft';
    set local role anon;
    select count(*) into v_n from app.question_modules where id = v_mod;
    reset role;
    v_txt := v_n::text;
    perform app.module_set_status('bygg-og-anlegg', '1.0.0', 'published');
    set local role anon;
    ...
    v_rows := v_rows || jsonb_build_object('seq', 3, 'name', 'anon sees no draft, and every row of a published module',
      'expected', '0|1|28', 'actual', v_txt, 'pass', v_txt = '0|1|28');

    -- 4 ---------------------------------------------------------------- published content is frozen
    v_txt := '';
    begin
      update app.module_items set text = '{"nb":"Endret"}' where module_id = v_mod and code = 'BA-SF-1';
      v_txt := 'updated';
    ...
    -- 6 ---------------------------------------------------------------- forward only
    begin
      perform app.module_set_status('bygg-og-anlegg', '1.0.0', 'draft');
      v_txt := 'moved';
    ...
    v_txt := app.module_seed(jsonb_build_object('module_id', 'bygg-og-anlegg', 'version', '1.0.0'), v_hash);
    begin
      perform app.module_seed(jsonb_build_object('module_id', 'bygg-og-anlegg', 'version', '1.0.0'), repeat('0', 64));
    ...
    select v_open, i.id, 'nei' from app.module_items i where i.module_id = v_mod and i.code = 'BA-T-1';
    v_txt := '';
    begin
    ...
    v_txt := coalesce(public.admin_module_set_status('bygg-og-anlegg', '1.0.0', 'retired', 'Ny versjon ute')->>'error', 'ok');
    perform set_config('request.jwt.claims', format(claims, v_sa, 'aal2'), true);
    v_txt := v_txt || ',' || coalesce(public.admin_module_set_status('bygg-og-anlegg', '1.0.0', 'retired', '')->>'error', 'ok');
    select count(*) into v_audit from app.admin_audit where action = 'module.retire';
    v_txt := v_txt || ',' || coalesce(public.admin_module_set_status('bygg-og-anlegg', '1.0.0', 'retired', 'Ny versjon ute')->>'error', 'ok');
  ...
  v_rows := v_rows || jsonb_build_object('seq', 13, 'name', 'every probe change was rolled back', 'expected', 'true',
    'actual', (not exists (select 1 from auth.users where email like '%@mod-test.example')
               and not exists (select 1 from app.round_modules where round_id = v_planned)
               and (select content_hash from app.question_modules where id = v_mod) = v_hash)::text,
    'pass', not exists (select 1 from auth.users where email like '%@mod-test.example')
               and not exists (select 1 from app.round_modules where round_id = v_planned)
               and (select content_hash from app.question_modules where id = v_mod) = v_hash);
...
begin
  ...
  if v_count <> 13 then raise exception 'module invariants: expected 13 rows, got %', v_count; end if;
end $$;
