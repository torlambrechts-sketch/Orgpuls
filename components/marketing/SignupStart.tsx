...
export function SignupStart({ label, submit, invalid }: { label: string; submit: string; invalid: string }) {
  ...
  return (
    ...
    >
      <label htmlFor={`${id}-orgnr`} className="mb-1 block text-mk-small font-bold">
        {label}
      </label>
      <div className="flex flex-wrap gap-3">
        <input
          ref={input}
          data-testid="orgnr-input"
          id={`${id}-orgnr`}
          name="orgnr"
          inputMode="numeric"
          autoComplete="off"
          ...
          className={`h-12 min-w-0 flex-[1_1_150px] rounded-tile border-2 bg-sf px-4 text-mk-lead font-semibold tracking-[0.06em] text-ink ${
            error ? 'border-danger' : 'border-line'
          }`}
        />
        <button
          type="submit"
          className="inline-flex h-12 flex-none cursor-pointer items-center rounded-tile border border-ink bg-ac px-6 text-mk-card font-bold text-ink"
        >
          {submit}
        </button>
      </div>
      {error ? (
        <p id={`${id}-error`} role="alert" className="mb-0 mt-2 text-mk-small font-semibold text-danger">
          {invalid}
        </p>
      ) : null}
    </form>
